...
(
  ...
  descr_categ VARCHAR(30)  NOT NULL
);  
...
(
  ...
  nome_usuario          VARCHAR(2)    NOT NULL,
...
(
  ...
  end_ediotra             VARCHAR(100) ,
  contato_editora         VARCHAR(30)
...
(
  ISBN              INTEGER(14)    NOT NULL,
  num_exemplar      INTEGER(2)     NOT NULL,
  num_edicao        INTEGER       NOT NULL,
  qtde_paginas      INTEGER,
  ano_publicacao    INTEGER       NOT NULL,
  idioma_exemplar   CHAR(20)      NOT NULL,
  valor_exemplar    DECIMAL(12,2)  NOT NULL,
  localizacao       CHAR(11)      CHECK(localizacao IN('Acervo','Circulante')),
  cod_origem        CHAR(1)       , #Já é criado a chave estrangeira, sem a necessidade do comando FOREIGN KEY.
  cod_livro         CHAR(10)      ,
  obs_exemplar      VARCHAR(100),
  situacao_exemplar CHAR(15)      NOT NULL,
  PRIMARY KEY (ISBN, num_exemplar), #criado a chave primária COMPOSTA
  CONSTRAINT FK_COD_ORIGEM FOREIGN KEY (cod_origem) REFERENCES origem_exemplar(cod_origem),
